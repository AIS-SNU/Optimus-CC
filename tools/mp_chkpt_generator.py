...
"""
Pretrained Model into Target Model Parallel Sized Pretrained Model

Megatron-LM only opened single gpu pretrained model.
This script divde sinlge or some sized pretrained model into \
target model parallel sized pretrained model.

Use 'examples/mp_chkpt_bert_generator.sh' to run.
...
def get_parallel_checkpoint_name(path):
    ...
    if metastring == 'release':
        checkpoint_name = get_checkpoint_name(path, iteration, release=True)
    else:
        checkpoint_name = get_checkpoint_name(path, iteration, release=False)
...
def get_mp_merge_args(parser):
    ...
    group.add_argument('--target-tensor-model-parallel-size', type=int, default=4,
...
def main():
    ...
    merged_model = get_model(model_type) # !!! This is the full model !!!

    # Tensor Model Parallel Model
    new_partitions = []
    # Set Total Tensor Model Parallel Size
    mpu.initialize.set_tensor_model_parallel_world_size(args.target_tensor_model_parallel_size)
    # [TODO] !!! Later support pipeline model parallel !!!
    mpu.initialize.set_pipeline_model_parallel_world_size(1)
    mpu.set_pipeline_model_parallel_rank(0)
    for i in range(0, args.target_tensor_model_parallel_size):
        mpu.initialize.set_tensor_model_parallel_rank(i)
        sub_model = get_model(model_type)
        new_partitions.append(sub_model)
    ...
    # Parameter generators so we can loop through them simultaneouly.
    merged_params_gen = merged_model.named_parameters() # full model
    partitions_params_gen = [partition.named_parameters()
                             for partition in partitions] # gathered model
    while True:
        try:
    ...
    # Now merged_model has real full model !!!

    new_partitions_params_gen = [partition.named_parameters() # splitted model
                            for partition in new_partitions]
    # We want to partition into our target tensor model parallel size
    while True:
        try:
            # Get the params and check names.
            name, merged_param = next(merged_params_gen)
            # merged_params_gen now has real weight values.
            print(' > working on {} ...'.format(name))
            print('     original weight      type: {}, size: {}'.format(
                merged_param.dtype, list(merged_param.size())))
            partitions_param = []
            for rank, new_partition_params_gen in enumerate(new_partitions_params_gen):
                partition_name, partition_param = next(new_partition_params_gen)
                assert partition_name == name
                partitions_param.append(partition_param)
                print('     partition {}    type: {}, size: {}'.format(
                    rank, partition_param.dtype, list(partition_param.size())))
            ...
            if not hasattr(merged_param, 'tensor_model_parallel'):
                print('     none-parallel parameter, simple copy from rank 0')
                with torch.no_grad():
                    partitions_param[0].data.copy_(merged_param.data)
                    # merged_param.data.copy_(partitions_param[0].data)
            # For parallel parameters, split the values 
            else:
                dim = merged_param.partition_dim
                stride = merged_param.partition_stride
                print(f'     parallel parameter split with stride {stride} along '
                      f'dimention {dim}')
                print('     split into {} partitions.'.format(args.target_tensor_model_parallel_size))
                new_partitions = split_into_partitions(merged_param,
                                                      args.target_tensor_model_parallel_size,
                                                      dim,
                                                      stride)
                for partition_, new_partition_ in zip(partitions_param, new_partitions):
                    partition_.data.copy_(new_partition_.data)
        except StopIteration:
            break

    iteration = 0
    mpu.initialize.set_tensor_model_parallel_world_size(args.tensor_model_parallel_size)
    for rank, model in enumerate(new_partitions):
        mpu.initialize.set_tensor_model_parallel_rank(rank)
        print(f"> saving rank {rank}'s model")
        save_checkpoint(iteration, [model], None, None, rank=rank)
    print('done :-)')
